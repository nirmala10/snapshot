...
{  
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = 'sites/all/modules/chat_master/bundle/jquery/css/base/jquery-ui.css';
    link.media = 'all';
...
if (!document.getElementById(cssId)){
    ...
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.href = 'sites/all/modules/chat_master/css/jquery.ui.chatbox.css';
    link.media = 'all';
    head.appendChild(link);
}
...
jQuery.when(
    $.cachedScript( "sites/all/modules/chat_master/bundle/io/build/iolib.min.js" ),
    $.cachedScript( "sites/all/modules/chat_master/build/chat.min.js" )

 ).done(function(){

     $.uiBackCompat = false;
     var userobj = {'userid':Drupal.settings.chat_master.id,'name':Drupal.settings.chat_master.name,'img':Drupal.settings.chat_master.imageurl};
     var room = 'main-c-room';//ToDo:

     userdata = {
            'userid':Drupal.settings.chat_master.id,
            'sid':'212',
            'rid': Drupal.settings.chat_master.path,
            'authuser':Drupal.settings.chat_master.auth_user,
            'authpass':Drupal.settings.chat_master.auth_pass,
            'userobj': userobj,
            'fastchat_lasttime':'0',
     ...
     $(document).ready(function(){
         ...
         if (localStorage.getItem(Drupal.settings.chat_master.sid) != null)  {
             displayChatHistory();
             vmstorage = JSON.parse(localStorage.getItem(Drupal.settings.chat_master.sid));
         }
